...
A snapshot of the development on the pandas project.
...
# Load data
with open("../devstats-data/pandas_issues.json", "r") as fh:
    issues = [item["node"] for item in json.loads(fh.read())]

glue("pandas_query_date", str(query_date.astype("M8[D]")))
```
...
glue("pandas_num_new_issues", len(newly_created))
glue("pandas_num_new_issues_closed", percent_val(len(new_issues_closed), len(newly_created)))
glue("pandas_new_issue_median_lifetime", f"{np.median(new_issue_lifetime)}")
```

{glue:text}`pandas_num_new_issues` new issues have been opened since
{glue:text}`pandas_query_date`, of which {glue:text}`pandas_num_new_issues_closed` have been
closed.

The median lifetime of new issues that were created and closed in this period
is {glue:text}`pandas_new_issue_median_lifetime`.
...
glue(
    "pandas_num_new_issues_responded",
    percent_val(len(commented_issues), len(newly_created_day_old))
)

glue("pandas_new_issues_at_least_1_day_old", len(newly_created_day_old))
glue("pandas_median_response_time", f"{median_time_til_first_response:1.0f}")
```

Of the {glue:text}`pandas_new_issues_at_least_1_day_old` issues that are at least 24
hours old, {glue:text}`pandas_num_new_issues_responded` of them have been commented
on.
The median time until an issue is first responded to is
{glue:text}`pandas_median_response_time` hours.
...
with open("../devstats-data/pandas_prs.json", "r") as fh:
...
plt.savefig("thumbs/pandas.png", bbox_inches="tight")
```
...
There have been a total of {glue:text}`pandas_num_merged_prs_with_known_authors`
merged PRs[^only_active] submitted by {glue:text}`pandas_num_unique_authors_of_merged_prs`
unique authors. {glue:text}`pandas_num_flyby` of these are "fly-by" PRs, i.e.
...
glue("pandas_num_merged_prs_with_known_authors", len(merged_prs))
glue("pandas_num_unique_authors_of_merged_prs", len(contributions_by_author))
glue("pandas_num_flyby", percent_val(num_flybys, len(num_merged_prs_per_author)))
```
...
is: {glue:text}`pandas_pony_factor`.
...
glue("pandas_pony_factor", pony_factor)
```
